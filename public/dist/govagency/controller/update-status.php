...
if (!in_array($action, ['approve', 'reject', 'suspend', 'unsuspend'])) {
    die("Invalid action.");
}

// Determine new status
$newStatus = ($action === 'approve' || $action === 'unsuspend') ? 'approved' : (($action === 'suspend') ? 'suspended':'rejected');
...
$stmt->close();
$location = ($action === 'suspend') ? '../suspended-accounts.php' : (($action === 'unsuspend') ? '../suspended-accounts.php' :'../account-approval.php');
