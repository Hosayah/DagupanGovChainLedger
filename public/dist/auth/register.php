...
<body class="bg-gray-100 flex justify-center items-center h-screen">
  <div class="floating-shapes" aria-hidden="true">
    <span
      style="background:#34d399; width:220px; height:220px; left:-60px; top:12%; animation: drift1 14s ease-in-out infinite;"></span>
    <span
      style="background:#10b981; width:160px; height:160px; right:-40px; top:34%; animation: drift2 18s ease-in-out infinite;"></span>
    <span
      style="background:#22c55e; width:260px; height:260px; left:18%; bottom:-90px; animation: drift3 22s ease-in-out infinite;"></span>
  </div>

  <div class="max-w-3xl bg-white shadow-lg rounded-xl mt-10" style="z-index: 1;">
    <h2 class="text-3xl font-bold text-center bg-green-600 w-full p-6 text-white" style="border-radius:12px 12px 0 0;">
    ...
    <div class="p-6">
      ...
      <div class="flex justify-center border-b mb-3">
        <button id="agency-tab" class="tab-btn px-4 py-2 text-green-600 border-b-2 border-green-600">Government
          Agency</button>
      ...
      <div class="p-6">
        ...
        <form id="agency-form" class="tab-content" method="POST" novalidate>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-2">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1" for="officeCode-agency">Office / Dept.
                Code</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1" for="agency-fullName">Full Name of
                Officer</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1" for="agency-govId">Government Employee
                ID</label>
          ...
          <button type="submit"
            class="w-full bg-green-600 text-white p-3 rounded-md hover:bg-green-700 transition">Register</button>
        </form>
        ...
        <form id="auditor-form" class="tab-content hidden" method="POST" novalidate
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-2">
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1" for="officeCode-auditor">Office / Dept.
                Code</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1" for="auditor-accreditation">Accreditation /
                License #</label>
            ...
            <div>
              <label class="block text-sm font-medium text-gray-700 mb-1" for="auditor-fullName">Representative
                Name</label>
            ...
            <div>
              ...
              <div class="relative pass">
                <input type="password" name="password" placeholder="Choose a strong password"
                  class="w-full p-3 border toggle-password border-gray-200 rounded-md bg-gray-50 focus:outline-none focus:ring-2 focus:ring-green-200"
                  required>
                <button type="button" aria-pressed="false" aria-label="Show password"
                  class="absolute inset-y-0 right-3 flex items-center text-gray-500 hover:text-green-600 focus:outline-none"
                  style="position: absolute; right: 0.75rem; top: 50%; transform: translateY(-50%);">
  ...
  <!-- SHOW PASSWORD TUNG TUNG SAGHUR -->
 
    <script>
(function(){
  // icon file paths (adjust if needed)
  const EYE = '../assets/images/eye.svg';
  const EYE_CROSSED = '../assets/images/eye-crossed.svg';

  // loop through all input fields
  document.querySelectorAll('input').forEach(input => {
    // find the matching toggle button in the same container
    const parent = input.parentElement;
    let btn = parent ? parent.querySelector('button') : null;
    if (!btn) {
      const rel = input.closest('.pass');
      btn = rel ? rel.querySelector('button') : null;
    }

    // skip if no related button found
    if (!btn) return;
    ...
    btn.addEventListener('click', () => {
      const container = btn.closest('.pass') || parent || btn.parentElement;
      const inputsToToggle = container ? Array.from(container.querySelectorAll('input')) : [input];

      // only toggle password-like inputs
      const togglable = inputsToToggle.filter(i => i.type === 'password' || i.type === 'text');
      const finalInputs = togglable.length ? togglable : [input];

      const isCurrentlyText = finalInputs.some(i => i.type === 'text');
      const show = !isCurrentlyText;

      finalInputs.forEach(i => i.type = show ? 'text' : 'password');

      // swap icon
      const img = btn.querySelector('img');
      if (img) img.src = show ? EYE_CROSSED : EYE;
