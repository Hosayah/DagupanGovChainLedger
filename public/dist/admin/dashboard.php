<?php
  ...
  $pendingUsers = $dao->getUsersByStatus('pending', 0);
  $counters = $dao->getUserCounters();
  
  $totalUsers = $counters['total'] > 0 ? $counters['total'] : 1; // avoid division by zero

  $approved_percentage = round(($counters['approved'] / $totalUsers) * 100, 2);
  $pending_percentage  = round(($counters['pending']  / $totalUsers) * 100, 2);

  $agency_percentage   = round(($counters['agency']   / $totalUsers) * 100, 2);
  $auditor_percentage  = round(($counters['auditor']  / $totalUsers) * 100, 2);
  $citizen_percentage  = round(($counters['citizen']  / $totalUsers) * 100, 2);
?>
  <!doctype html>
              ...
              <div class="card-body">
                <div class="flex items-center justify-between gap-3 flex-wrap">
                  <h3 class="font-light flex items-center mb-0">
                    ...
                    <?= htmlspecialchars($counters['approved']) ?>
                  </h3>
              ...
              <div class="card-body">
                <div class="flex items-center justify-between gap-3 flex-wrap">
                  <h3 class="font-light flex items-center mb-0">
                    ...
                    <?= htmlspecialchars($counters['pending']) ?>
                  </h3>
